...
Analyze `awake:engine:ui-unstyled`' pixel-baseline UI snapshot gallery for legibility/occlusion
...
```bash
./gradlew :awake:engine:ui-unstyled:desktopTest
```

This regenerates every PNG under `awake/engine/ui-unstyled/build/ui-snapshots/*.png` and,
via the `uiSnapshotReport` task (`finalizedBy desktopTest`), a self-contained HTML gallery
at `awake/engine/ui-unstyled/build/reports/ui-snapshots/index.html` -- unconditionally,
pass or fail.
...
```bash
find "${ARGUMENTS:-.}/awake/engine/ui-unstyled/build/ui-snapshots" -name "*.png" | sort
```
...
- This is a supplement to, not a replacement for, `awake:engine:ui-unstyled`' own pass/fail unit
