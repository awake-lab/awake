 ...
 *
 * Scoped to the characters this repo's own UI labels actually use (not full ASCII) --
 * extending coverage means adding another row to [GLYPH_ROWS] below, each entry a `Char` to
 * 8 row-bitmasks (bit 7 = leftmost pixel). Builds its own atlas pixel buffer at
