 ...
 * -- [io.github.ronjunevaldoz.awake.vulkan.texture.Texture] wants raw interleaved RGBA8
 ...
 * Vulkan-backed [Texture], reusing the same [createBitmap] decode path the legacy OpenGL
 * [TextureLoader] already uses (see that object's doc comment) so there's exactly one
 * "bytes -> decoded image" step in this engine, not two independently-maintained ones.
 *
