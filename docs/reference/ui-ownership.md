...
- `ui-designsystem` owns named themes, token instances, component recipes, and branded variants.
  ...
  `shadcnThemeValues(...)` is the pure, unscoped value factory. Its `ShadcnThemeExtension` is the
  design-system-owned, root-scoped seam for named-role customization; public recipes expose
