...
| `awake:engine:game-dsl` | Dokka + `awake/engine/game-dsl/README.md` | Partial | `samples:hello-cube` authored bootstrap proof | In progress | Expand `game {}` cookbook and installer patterns |
...
| `awake:scene-dsl` | Dokka + `awake/scene-dsl/README.md` | Partial | `game { ecs(...) }` composition proof | In progress | Expand scene authoring cookbook |
...
1. `ui-core`, `ui-widgets`, `ui-designsystem`
