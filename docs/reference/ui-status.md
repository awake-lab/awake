...
| 5 | Three spacing vocabularies | `UiSpacing` / `ShadcnSpacing` / `Tw` | **Partially done 2026-08-18.** `ShadcnSpacing` deleted — its only reference was the unused `ShadcnResolvedTheme.spacing` property (zero real call sites; `theme.spacing.xxl` in its own doc comment was aspirational, not real). `Tw` was already correctly confined to designsystem. `UiSpacing`'s final resting place is unresolved: the audit's B12 row was revised same-day to "delete `UiSpacing` too, inline literal `Dp` at every core/headless call site" — a materially larger, different sweep than "relocate to core/headless" (the decision this pass was scoped against). Not attempted here; needs its own pass once the two conflicting decisions in the audit doc are reconciled. `ShadcnSurfaceStyles.kt`'s `shadcnLegacyAmbientSurfaceStyle` still calls `UiSpacing.sm` deliberately (reproduces ui-core's former ambient default, not a shadcn-branded value) and would need reworking either way. The "4 known-wrong values" cited at `ShadcnInputOtpRecipes.kt`/`ShadcnVariants.kt` no longer exist at those sites; both now use `Tw`-based `.tw` units or plain literal `Dp` (E6, parked). |
