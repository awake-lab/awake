...
| F11 | Pointer pipeline stores latest state only — a mousedown+mouseup inside one frame nets to nothing (sub-frame taps, synthetic clicks physically droppable; carried from 2026-08-15 audit). Also the prerequisite for long-press/double-click recognition in the C9 gesture contract | wasm host `bindWindowPointerInput` | 2 |
