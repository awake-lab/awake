...
|---|---|---|---|
| Delete headless facade mirror | B9 | **Scoped 2026-08-18, not landed — real blast radius measured, 3x the original estimate.** `HeadlessModifier` isn't "16 forwards", it's a full duplicate builder API (~20+ functions: width/height/size/padding/margin/offset/weight/align/clickable/fillMax*/etc, plus an `Arrangement` mirror) implementing a real `Modifier` interface, wrapping Core's `UiModifier` (also a real class with identically-named builder extensions — mechanically mergeable via typealias). `UiScope` is NOT a redundant wall — its docstring states real intent: capability narrowing, hiding Core's raw `UiPrimitiveScope` frame/draw/input escape hatches from widget-recipe authors. Measured: 406 headless-`Modifier` occurrences/44 files, 130 files importing headless directly, 46 files referencing headless `UiScope`, and **274 bypass-import lines across 140 files** reaching past headless into `ui-core` directly (audit's "48" was a large undercount). Proposed 3-way split: **B9a** — **attempted 2026-08-18, STOPPED, real language-level blocker (not mechanical after all).** Headless's `Modifier` is used both as a type AND as a bare value (`modifier: Modifier = Modifier`, `Modifier.fillMaxWidth()...`), which only works because `interface Modifier { companion object : Modifier }` makes the type name resolve to a value too. Core's `UiModifier` is a `data class` — can't be subclassed (so no companion-object trick), and Kotlin forbids a top-level `typealias Foo` and `val Foo` sharing a name in one package (confirmed experimentally, real "Conflicting declarations" error). A bare typealias merge is impossible without either making `UiModifier` open/interface-shaped (a Core class-shape change touching every `.copy()`-based builder — out of scope) or rewriting 238+ bare-`Modifier`-value call sites tree-wide (an order of magnitude bigger than described). Real unblock needs a Core-side decision: give `UiModifier` a real companion-object default, or make headless's wrapper a cheap `@JvmInline value class` instead of full duplication. No code changed, reverted clean. **B9b** repoint the 274 bypass imports onto the headless facade, or formally bless direct `ui-core` imports for the `.primitive`-escape-hatch sites and let `verifyUiOwnership` + `implementation(ui-core)` enforce instead (closing the doc/reality gap rather than 274 call sites by hand) — not attempted. **B9c** — real design work, not mechanical: decide whether `UiScope`'s capability-narrowing stays, and if so whether it's better expressed as a Core-side narrower capability interface than headless re-wrapping it — not attempted, needs a design call | Touches every widget signature; boundary is better enforced by package-1's live check + `implementation(ui-core)`. B9a needs a Core `UiModifier` shape decision before any code moves; B9b/B9c also not yet launched |
