...
- [x] `id: String` required on designsystem recipes following headless's convention; 9 `Unit`
      ...
      Unit→UiBounds fixes whose body already tail-calls a UiBounds-returning headless primitive
...
| Layout module placement | P3 (new, 2026-08-18) | `column`/`row`/`box`/`Modifier`/`Arrangement` layout math currently lives inside `ui-core` alongside rendering/input/state; `ui-headless` re-exports it under its own package for consumers. **Real Compose precedent checked**: `Column`/`Row`/`Box`/`Arrangement` live in `androidx.compose.foundation.layout`, the SAME `compose-foundation` artifact that owns unstyled interaction behavior (`clickable`, `BasicText`) — Compose does not split layout into its own module. Only `Modifier` (the interface) sits one layer below, in `compose-ui` (≈ `ui-core`). Mapped onto Awake, Compose's own layering argues layout should move fully INTO `ui-headless` (compose-foundation's shape), not out to a third module | Not sized — needs its own scoping pass (what exactly moves, migration blast radius across headless/designsystem/samples) before any agent starts it; raised but explicitly deferred 2026-08-18 |
...
| Runtime duplicate-id collision check | P5 (new, 2026-08-20) | Investigated whether a runtime check can catch a caller-supplied literal `id` reused across two sibling widget instances (the case the required-`id` fix, `c1a6dab10`, does NOT close — that pass only closed the "forgot to pass an id, silently fell to a shared default" case). **Trial-pass signal already exists and is clean**: a wrap-content/scroll sizing trial always runs on a *separate* `UiContext(measuring = true, ...)` instance (`UiContextMeasureState.createMeasureContext`), and `context.isMeasuringInternal()` is the exact, already-used gate for "is this a real paint-committing frame or a trial re-run" — `tryClaimActiveInternal`/`requestCursorInternal`/`emitInternal`/`hitTest` etc. all already gate on `!measuring`, so a duplicate-id check reusing that same gate would not false-positive on trial re-visits. **But that isn't the real blocker.** `widgetStateInternal(id)` — the natural hook, since it's the one call every stateful widget routes through — is called from *multiple call sites within one real, non-measuring render of a single logical widget instance* (confirmed: `TextField.kt:161` and `:331` both call `widgetState(id)` with the identical literal `id` from two different functions inside one `textField()` render). A naive "same id claimed twice in one real frame ⇒ throw" at `widgetStateInternal` would false-positive on that legitimate pattern, not just on trial passes. Distinguishing "one instance calling its own id twice (fine)" from "two sibling instances colliding on the same literal id (the bug)" needs a widget-*instance* boundary marker — nothing today plays that role; `enterLayoutNodeInternal`/`exitLayoutNodeInternal` (`UiContext.kt`) brackets row/column layout nodes for positional weight-memory, not arbitrary id-bearing widgets, and this DSL has no call-site identity to fall back on (`UiContext.kt`'s own comment on `pathHashStack`). Checking at `tryClaimActiveInternal` instead (called once per real frame per interactive widget, from `ClickableModifiers.kt`/`Interaction.kt`) avoids the multi-call problem but has a coverage gap: none of the 4 widgets `docs/audits/2026-08-17-...md` flagged as real id-collision bugs (`shadcnAvatarBadge`, `shadcnAvatarGroup`, `shadcnFieldSeparator`, `shadcnSidebarMenuSub`) are clickable, so the actual bug class this task is about would sail through undetected. No existing debug-only-assertion convention was found in `ui-core` to gate a hard throw on (`grep DEBUG/assert` empty); the codebase's existing hard-throw precedent (`Dimension.resolve`'s `error("WrapContent must be resolved...")`, `requireScrollableContainer`) fires at one-time layout-definition points, not in a per-real-frame hot path, so its perf profile doesn't transfer. **Verdict: not safe to implement yet.** The trial-vs-real-frame axis is solved; the actual missing piece is a new "widget instance" open/close bracket (a real new primitive, not a modifier) that every id-taking leaf recipe would need to adopt at its single entry point to get correct duplicate semantics — real surface area is every `shadcn*`/headless component with an `id` param (dozens, not the 5 already fixed), not a 5-minute follow-up | Not a small addition; needs a real instance-boundary primitive designed and then wired through every id-taking widget before a duplicate check can fire without false positives — routed here as a scoped-later design task, not attempted |
