...
import io.github.ronjunevaldoz.awake.engine.platformauthoring.ui.AppUiRuntime.Companion.EMPTY_UI_ONLY_CAMERA
import io.github.ronjunevaldoz.awake.engine.platformauthoring.ui.AppUiRuntime.Companion.FRAME_TIME_HISTORY_SIZE
...
class AppUiRuntime(
    ...
    /** Public (not private-set) so an installing [io.github.ronjunevaldoz.awake.engine.app.core.AppModule] -- see [provideDrawCalls]'s doc
    ...
    /** Optional per-frame hook an installing [io.github.ronjunevaldoz.awake.engine.app.core.AppModule] sets (from its own `overlay`/
     ...
     * own, so this is the only way a demo/game built on [AppUiRuntime] gets real mesh geometry
...
data class AppUiSpec(
    val theme: UiTheme,
    val font: UiFont,
    val overlays: List<AppUiOverlayBlock>,
    val onReadyBlock: AppUiReadyBlock,
    val onDisposeBlock: AppUiDisposeBlock,
) : AppInstaller {
    override fun install(into: AppSpecBuilder) {
        val runtime = AppUiRuntime(into.serviceLookup(), this)
        into.service(AppUiRuntime::class, runtime)
...
/** Root-level full-viewport box for a [AppUiRuntime] overlay -- named `frame`, not `canvas`,
...
fun AppUiRuntime.headlessFrame(
    block: UiScope.() -> Unit,
) {
    val rootSlot = UiBounds(0f, 0f, viewportWidth, viewportHeight)
