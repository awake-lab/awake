...
class SliderEnabledTest {
    ...
    fun dragMovesValueWhenEnabled() {
        ...
        assertEquals(100f, value, "dragging from the left edge to the right edge must reach max when enabled (the default)")
    }
    ...
    fun dragDoesNotMoveValueWhenDisabled() {
        ...
        assertEquals(0f, value, "enabled = false must suppress the drag even on an otherwise valid press+move+release")
    }
}
