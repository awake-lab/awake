 ...
 * [Renderer.draw] expects, same "world state in, render-api call out" shape the mesh/camera
 * side already has. A recording fake [Renderer] (matching [Scene3DPlaygroundUiTest]'s own
 * `RecordingScene3DRenderer` shape) captures what actually reached [Renderer.draw] without
...
class RenderSystemTest {
    private class RecordingRenderer : Renderer {
        ...
        override fun draw(camera: CoreCamera, drawCalls: List<DrawCall>, light: SceneLight) {
            lastLight = light
        }
        override fun renderToTexture(target: RenderTarget, camera: CoreCamera, drawCalls: List<DrawCall>) = Unit
