...
internal fun shadcnAlertDialogSurfaceStyle(values: UiThemeValues, metrics: ShadcnMetrics): Style = Style { background(values.colors.card); foreground(values.colors.cardForeground); border(1f.dp, values.colors.border); shape(values.shapes.lg); contentPadding(metrics.panelPadding) }
