...
internal fun shadcnDropdownSurfaceStyle(values: ShadcnThemeValues): Style = Style { background(values.colors.popover); foreground(values.colors.popoverForeground); border(1f.dp, values.colors.border); shape(values.shapes.md); contentPadding(4f.dp) }
...
internal fun shadcnTooltipStyle(values: ShadcnThemeValues): Style = Style { background(values.colors.foreground); foreground(values.colors.background); shape(values.shapes.md); contentPadding(horizontal = 12f.dp, vertical = 6f.dp); textSize(12f.sp); lineHeight(16f.sp) }
internal fun shadcnAlertDialogSurfaceStyle(values: ShadcnThemeValues, metrics: ShadcnMetrics): Style = Style { background(values.colors.card); foreground(values.colors.cardForeground); border(1f.dp, values.colors.border); shape(values.shapes.lg); contentPadding(metrics.panelPadding) }
