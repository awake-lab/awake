...
class StudioGizmoWiringTest {
    ...
    fun clickingInsideTheViewportSelectsAndClickingOutsideItDoesNot() = runTest {
        ...
        click(game, input, inspector.bounds.x + inspector.bounds.width / 2f, inspector.bounds.y + 40f)
        assertEquals(null, store.state.value.inspector.selectedEntityId, "a click outside must not select")

        // The scene is framed on its camera target, so the middle of the viewport is over geometry.
        click(game, input, viewport.bounds.x + viewport.bounds.width / 2f, viewport.bounds.y + viewport.bounds.height / 2f)
        ...
        runtime.world.queryEach<Name> { entity, name -> if (entity.id == selected) selectedName = name.value }
        assertTrue(selectedName == "cube" || selectedName == "ground", "selected $selectedName")
    }

    private suspend fun click(game: Game, input: Input, x: Float, y: Float) {
