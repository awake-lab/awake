...
/**
 * A rail card: `shadcnSurface`'s Default look, but padded by [RailPadding] instead of the
 * theme's panel padding.
 *
 * `shadcnSurface`'s Default variant is a PANEL -- 16dp of content padding on every side. A rail
 * is [RailWidth] (= one 32dp icon button plus 4dp either side) wide, so that padding left an 8dp
 * interior for a 32dp button: every icon overflowed to the right, half-clipped by the card, and
 * the card stood ~60dp taller than its two buttons. Same class of bug as `barBand` in
 * StudioToolbar.kt, whose doc comment spells out the panel-vs-band distinction -- a card's
 * declared width must fit its content.
 */
private fun UiScope.railCard(id: String, content: ColumnScope.() -> Unit): UiBounds = shadcnSurface(
    id = id,
