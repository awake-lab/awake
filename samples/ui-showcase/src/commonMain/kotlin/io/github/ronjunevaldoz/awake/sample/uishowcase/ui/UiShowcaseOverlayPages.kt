...
internal fun ColumnScope.drawUiShowcasePopupPreview() {
    ...
    shadcnCard(id = "showcase-popup-card", modifier = Modifier.height(120f.dp)) {
        shadcnText("Popup behavior is supplied by the Headless popup primitive and can be skinned independently.")
    }
}
...
internal fun ColumnScope.drawUiShowcaseTooltipPreview() {
    ...
    spacer(Modifier.height(8f.dp))
    shadcnMuted("Tooltips use an anchor slot and a neutral popup result; the recipe owns only shadcn visuals.")
}
...
internal fun ColumnScope.drawUiShowcasePopoverPreview() {
    ...
    spacer(Modifier.height(8f.dp))
    shadcnMuted("Popover placement and dismissal remain Headless behavior.")
}
