 ...
 * [io.github.ronjunevaldoz.awake.ui.designsystem.awakeShadcnPropertyTextField] wrapper -- across
...
class UiShowcaseTextInputIntegrationTest {
    ...
    fun clickingTheRealPageFieldGrantsFocusAndRendersTypedGlyphs() {
        ...
        val nameField = ui.semanticNodes().firstOrNull { it.role == UiSemanticRole.Text && it.id == "showcase-name" }
        ...
        assertTrue(glyphCount > 0, "typed text must actually render as glyph primitives, not just update hidden state")

        val nameLabel = ui.semanticNodes().firstOrNull { it.role == UiSemanticRole.Text && it.id == "showcase-name" }?.label
        assertEquals("Hi", nameLabel, "the field's value must reflect what was typed after going through the real page composition")
    }
    ...
    fun typedTextIsIgnoredBeforeAnyFieldIsClicked() {
        ...
        val label = ui.semanticNodes().firstOrNull { it.role == UiSemanticRole.Text && it.id == "showcase-name" }?.label
        assertEquals("Jane Doe", label, "typed text must not leak into a field that was never clicked -- value stays empty, so its placeholder is still what's recorded")
    }
}
